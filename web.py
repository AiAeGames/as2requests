...
def index(where='0'):
    ...
    if checker(api=api) == True:
        ...
        info = result
        temp_request = {}
        res = execute("SELECT * FROM requests WHERE user_id = %s and action = %s ORDER BY id and action = 0 asc", [info["id"], where])
        ...
        for row in results:
            ...
            temp_request["duration"] = row["duration"].split('PT')[1]
    ...
    return render_template('index.html', path=request.url_root, url=request.url, userinfo=info, list_requests=list_requests, where=where, stats=counter(info["id"]))

@app.route('/action/<action>/<rid>')
def action_delete(action, rid):
    api = request.cookies.get('api')
    q = execute("SELECT * FROM users WHERE api=%s", [api])
    result = q.fetchone()
    if action == "done":
...
if __name__ == "__main__":
    ...
    #ssl_context=context
    app.run(debug=True, port=7294, threaded=False, host='0.0.0.0')